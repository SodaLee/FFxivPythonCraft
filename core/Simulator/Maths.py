...
def base_progress(craft: int, clv: int, rlv: int):
    return lv_dif_progress(clv, rlv) * (0.21 * craft + 2) * (10000 + craft) / (10000 + Definitions.suggestProp[rlv][0])


def base_quality(control: int, clv: int, rlv: int):
    return lv_dif_quality(clv, rlv) * (0.35 * control + 35) * (10000 + control) / (10000 + Definitions.suggestProp[rlv][1])
